...
export default async function loadShapefile(filePath: string): Promise<Shapefile> {
  try {
    ...
    // 파일들을 fetch로 로드
    const [shpResponse, dbfResponse] = await Promise.all([fetch(shpPath), fetch(dbfPath)]);
    ...
    // SHP와 DBF 파일 파싱
    const [shpData, dbfData] = await Promise.all([parseShp(shpBuffer), parseDbf(dbfBuffer)]);
